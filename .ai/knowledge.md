...
| Version | ADempiere base version `3.9.4`; tags include `adempiere-3.9.3-1.3.6` and `adempiere-3.9.4-1.3.6` |
...
The repository owns a reusable ADempiere withholding engine. It provides dictionary structures, a model validator, document/process validation, withholding generation, declaration, reprocessing, and sending, together with the extension point for location-specific calculations.
...
- Migrations under `xml/migration/` are numbered sequence files that create dictionary records, windows, processes, smart browsers, and related metadata.
...
| Publish workflow uses multiple credential-bearing environment variables and fallback tokens. | Missing or misconfigured values can fail publish with a confusing 401. | Validate publish credentials and `packages: write` permission before release; keep values out of the repository. |
...
CI builds on push and pull requests. Release publication exists for Sonatype and GitHub Packages. Platform workflows cover knowledge-on-release, pull-request review, and release candidates.
